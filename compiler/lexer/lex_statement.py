...
def lex_statement(string: str, token_list: List[Tuple[LT, Any, Union[int, Tuple[int, int]]]]) -> int:
    ...
    while index < len(string):
        ...
        if c == " " or c == LINE_BREAK:
            ...
            if len(expr) > 0:
                lraise(SyntaxError(f"Arg open '{ARG_OPEN}' is not allowed to be preceded by anything but a space."),
                       char(string[index:]))
            ...
            if not c.isalpha() and not c.isdigit() and c not in PATH_CHARS:
                lraise(SyntaxError(f"Statement Keyword must be made up of only alphabetical characters and not '{c}'"),
                       char(string[index:]))
            expr += c
